...
module.exports = async function handler(req, res) {
  ...
  try {
    if (req.method === 'GET') {
      const { data, error } = await sb.from('matches').select('id, data').order('created_at', { ascending: false });
    ...
    if (req.method === 'POST') {
      ...
      if (action === 'upsert') {
        ...
        const rows = items.map(m => ({ id: m.id, data: m, updated_at: new Date().toISOString() }));
      ...
      if (action === 'import') {
        ...
        const rows = matches.map(m => ({ id: m.id, data: m, updated_at: new Date().toISOString() }));
        const { error } = await sb.from('matches').upsert(rows);
        if (error) throw error;
